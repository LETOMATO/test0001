...
{ 	
    ...
    else
    {
        ...
        while(i<10)
        {
			if(strcmp(p->dif[i].name,posdoc)==0){p->dif[i].locs[p->dif[i].freq]=loc;p->dif[i].freq+=1;return -1;}
			i++;
		}
...
void daopai()
{
	...
	struct Node*p;//倒排表指针
	char *txt[]={"1.txt","2.txt","3.txt","4.txt","5.txt","6.txt"};
	int shumu=0;
	for(unsigned int i=0;i<(sizeof(txt)/sizeof(*txt));i++){
		add_txtNode(txt[i]);
		shumu++;
	}
...
//	}
	...
	for(int i=0;i<shumu;i++){
		char files[50];
		strcpy(files,"C:\\Users\\asus\\Desktop\\git-test001\\内容索引子系统\\");
		strcat(files,f->name);
